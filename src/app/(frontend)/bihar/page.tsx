...
import { FALLBACK_DATASETS, FALLBACK_DISTRICTS } from "@/lib/data";
import type { BiharDataset, DistrictFactsheet } from "@/lib/data";
...
export default async function BiharObservatoryPage() {
  const [datasetsRaw, districtsRaw] = await Promise.all([
    prisma.biharDataset.findMany({ orderBy: { year: 'desc' } }),
    prisma.districtFactsheet.findMany({ orderBy: { districtName: 'asc' } })
  ]);

  const datasets = datasetsRaw.map(d => ({ ...d, slug: d.slug || undefined }));
  const districts = districtsRaw.map(d => ({ ...d, slug: d.slug || undefined }));
