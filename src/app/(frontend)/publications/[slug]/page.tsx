...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const pubData = await prisma.publication.findUnique({ where: { slug } });
...
export default async function PublicationDetailPage({
  ...
  const publicationRaw = await prisma.publication.findUnique({
    where: { slug },
    include: { authors: { include: { author: true } }, researchVertical: true }
  });
  
  if (!publicationRaw) return notFound();

  const publication = {
    ...publicationRaw,
    authors: publicationRaw.authors.map(pa => pa.author),
    slug: publicationRaw.slug || undefined,
  };

  const allPublicationsRaw = await prisma.publication.findMany({
    where: { slug: { not: slug } },
    include: { authors: { include: { author: true } }, researchVertical: true },
    take: 3
  });

  const allPublications = allPublicationsRaw.map(pub => ({
    ...pub,
  ...
  const relatedList = (allPublications && allPublications.length > 0)
    ? allPublications.filter(p => p.slug !== slug)
    : [];
